  ...
  try {
    ...
    if (amount) {
      await Payment.create({
        ...
        currency: 'INR',
        provider: 'cash',
        method: 'upi', // schema requires enum; store 'upi' as default even for cash
        status: 'success',
...
exports.verify = async (req, res, next) => {
  try {
    ...
    const ride = await Ride.findByIdAndUpdate(rideId, { $set: { paymentStatus: 'completed', paymentMethod: 'online' } }, { new: true });

    // Credit rider wallet with 90% (admin 10% cut)
    try {
      const amount = Number(paymentDoc?.amount || ride?.finalPrice || 0);
      const adminShare = Math.round(amount * 0.10);
